...
module.exports = {
    init: function(args) {
        ...
        if( args[2] == '-b' && typeof(args[3]) == 'string' && args[4] == 'webpack'){
             //console.log('current: '+process.release.libUrl);
             fs.readFile(process.env.PWD+"/template/_webpack.js", 'utf8',function(err,data){
                if (err) throw err;
             ...
             fs.readFile(process.env.PWD+"/template/_base.html", 'utf8',function(err,data){
                if (err) throw err;
                var sDirurl = process.env.PWD + '/news/pages/' + args[3];
                file.mkdir(sDirurl);
                //console.log(data)
             ...
             fs.readFile(process.env.PWD+"/template/_entry.js", 'utf8',function(err,data){
                if (err) throw err;
                var sDirurl = process.env.PWD + '/news/pages/' + args[3];
                file.mkdir(sDirurl);
                //console.log(data)
